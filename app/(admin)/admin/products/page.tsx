  ...
  const refetchProducts = async () => {
    ...
    try {
      const [productsRes, categoriesRes] = await Promise.all([
        productsApi.list({ search: search || '', category: categoryFilter || '', page: '1', limit: '200' }),
        categoriesApi.list(),
      ]);

      const productsList = Array.isArray(productsRes?.data) ? productsRes.data : Array.isArray(productsRes) ? productsRes : [];
      const categoriesList = Array.isArray(categoriesRes?.data) ? categoriesRes.data : Array.isArray(categoriesRes) ? categoriesRes : [];

      setProducts(productsList.map((p: any) => ({ ...p, id: p._id || p.id })));
      setCategories(categoriesList.map((c: any) => ({ id: c._id || c.id, name: c.name })));
    } catch (err) {
      console.error('Failed to fetch admin products/categories', err);
      setProducts([]);
      setCategories([]);
    } finally {
      setLoading(false);
