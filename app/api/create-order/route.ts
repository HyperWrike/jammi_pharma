...
async function createRazorpayPaymentLink(args: {
  ...
  if (!keyId || !keySecret) {
    return { data: null as any, error: 'Missing Razorpay credentials (RAZORPAY_KEY_ID / RAZORPAY_KEY_SECRET).' };
  }

  const amount = Math.max(1, Math.round(Number(args.amountInRupees || 0) * 100));
  const callbackUrl = `${args.callbackBaseUrl}/checkout?order=${encodeURIComponent(args.orderNumber)}`;
  ...
  const payload = {
    ...
    description: `Jammi Order ${args.orderNumber}`,
    customer: {
  ...
  if (!res.ok) {
    ...
    return { data: null as any, error: `Razorpay link creation failed: ${raw}` };
  }
  ...
  return {
    data: {
      ...
      shortUrl: json?.short_url as string | undefined,
    },
...
async function createRazorpayOrder(args: {
  ...
  if (!res.ok) {
    const raw = await res.text();
    return { data: null as any, error: `Razorpay order creation failed: ${raw}` };
  }
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    let razorpayOrderOrLinkId: string | null = null;
    let razorpayOrderPayload: {
      id?: string;
      amount?: number;
      currency?: string;
      keyId?: string;
    ...
    if (paymentMethod !== 'COD') {
      const callbackBaseUrl = process.env.NEXT_PUBLIC_SITE_URL || req.nextUrl.origin;
      ...
      paymentLinkUrl = linkResult?.data?.shortUrl || null;
      razorpayOrderOrLinkId = linkResult?.data?.id || null;
      paymentError = linkResult?.error || null;
      ...
      if (!paymentLinkUrl) {
        ...
        razorpayOrderPayload = orderResult?.data || null;
        razorpayOrderOrLinkId = orderResult?.data?.id || razorpayOrderOrLinkId;
        if (orderResult?.error) {
          paymentError = [paymentError, orderResult.error].filter(Boolean).join(' | ');
        }
      }

      if (razorpayOrderOrLinkId) {
