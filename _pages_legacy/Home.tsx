...
const Home: React.FC = () => {
  const essentialsProducts = useMemo(() => {
    const preferred = MOCK_PRODUCTS.filter((p) =>
      ['triphala-churna', 'yummunity', 'trip-caps', 'daily-dew', 'livercure', 'kesh-pro-oil'].includes(p.id)
    );
    return preferred.length > 0 ? preferred : MOCK_PRODUCTS.slice(0, 6);
  }, []);

  const [essentialsStart, setEssentialsStart] = useState(0);
  const essentialsPerView = 3;
  const maxEssentialsStart = Math.max(0, essentialsProducts.length - essentialsPerView);
  const visibleEssentials = essentialsProducts.slice(essentialsStart, essentialsStart + essentialsPerView);

  const { content, loading } = useCMSContent('homepage');

  const heroImageUrl = '/images/Finalized Logo.png';
  const claimImageUrl = '/images/tamil_doctor_lab.png';

  const cleanEssentialsCta = String((content as any)?.essentials?.ctaText || 'Shop with Us')
    .replace(/https?:\/\/\S+/gi, '')
    .replace(/\s+/g, ' ')
    .trim();
  ...
  return (
    <div className="bg-[var(--purple)] pt-[5rem]">

      {/* Section 1: Hero */}
      <section className="relative min-h-[640px] lg:min-h-[750px] overflow-hidden bg-[var(--purple)] lg:flex lg:items-stretch">
        ...
        <div className="w-full lg:w-1/2 bg-[var(--purple)] flex items-center">
          <div className="max-w-[560px] px-5 sm:px-10 lg:px-14 py-12 lg:py-16">
            <h1 className="m-0 leading-[0.95]">
              <div className="text-[40px] sm:text-[56px] lg:text-[72px] font-black text-[var(--yellow)] tracking-tight inline-block">
                <CMS section="hero" field="titleLine1" fallback="INDIA'S" />
              </div>
              <div className="text-[34px] sm:text-[44px] lg:text-[52px] font-normal italic text-white leading-[1.0] mt-2 inline-block">
                <CMS section="hero" field="titleLine2" fallback="Healthcare" />
              </div>
              <div className="text-[40px] sm:text-[56px] lg:text-[72px] font-black text-[var(--orange)] tracking-tight mt-2 inline-block">
                <CMS section="hero" field="titleLine3" fallback="RENAISSANCE" />
              </div>
            </h1>

            <p className="text-base sm:text-lg lg:text-[19px] text-white/95 max-w-[520px] leading-relaxed mt-6 sm:mt-8 font-normal">
              <CMS section="hero" field="subtitle" multiline fallback="We are not 'Alternative Medicine.' We are India's primary healthcare solution since 1897." />
            </p>
        ...
        {/* Right Column (Restored with Product Image) */}
        <div className="w-full lg:w-1/2 relative flex items-center justify-center overflow-visible px-5 pb-10 lg:px-0 lg:pb-0"
             style={{ background: "radial-gradient(circle at 35% 20%, rgba(249,209,57,0.40) 0%, rgba(249,209,57,0.00) 60%), #540C3C" }}>
          <div className="absolute inset-0 pointer-events-none">
            <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[360px] h-[360px] sm:w-[460px] sm:h-[460px] lg:w-[520px] lg:h-[520px] rounded-full"
                 style={{ background: "radial-gradient(circle, rgba(249,209,57,0.55) 0%, rgba(249,209,57,0.0) 65%)", filter: "blur(8px)" }} />
          </div>
          <div className="relative w-full max-w-[420px] sm:max-w-[460px]">
            <img alt="Jammi product" src="/images/Yummunity Bottle.png" className="w-full h-auto object-contain rounded-[18px]" style={{ boxShadow: "0 18px 45px rgba(0,0,0,0.22)" }} />
          </div>
        </div>
      </section>
      
      {/* Container for the rest to preserve light background */}
          ...
          <div className="relative mx-auto mt-12 lg:mt-0 w-full max-w-lg lg:max-w-full rotate-2 hover:rotate-0 transition-transform duration-700">
            ...
            <div className="relative z-10 w-full aspect-[4/5] object-cover shadow-2xl overflow-hidden">
              <LiveEditable
                page="homepage"
                section="claim"
                contentKey="imageUrlV2"
                initialContent={claimImageUrl}
                type="image"
      ...
      <section className="bg-background-light py-16 sm:py-20 lg:py-32 border-b border-cream-dark border-t border-cream-dark">
        <div className="max-w-7xl mx-auto px-6 lg:px-10">
          <div className="flex flex-col md:flex-row justify-between md:items-end gap-6 mb-12 sm:mb-16">
            ...
            <Link href="/shop" className="border-2 border-forest text-forest font-bold tracking-wide text-sm px-6 sm:px-8 py-3 sm:py-4 hover:bg-forest hover:text-white transition-colors w-full sm:w-auto text-center">
              <LiveEditable
                page="homepage"
                section="essentials"
                contentKey="ctaText"
                initialContent={cleanEssentialsCta || 'Shop with Us'}
                type="text"
          ...
          <div className="flex items-center justify-end gap-3 mb-6">
            <button
              type="button"
              onClick={() => setEssentialsStart((prev) => Math.max(0, prev - essentialsPerView))}
              disabled={essentialsStart === 0}
              className="w-10 h-10 rounded-full border border-forest/30 text-forest disabled:opacity-40 disabled:cursor-not-allowed"
            ...
            <button
              type="button"
              onClick={() => setEssentialsStart((prev) => Math.min(maxEssentialsStart, prev + essentialsPerView))}
              disabled={essentialsStart >= maxEssentialsStart}
              className="w-10 h-10 rounded-full border border-forest/30 text-forest disabled:opacity-40 disabled:cursor-not-allowed"
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-10 grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          ...
          <div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-4">
              ...
              <Link href="/shop" className="bg-transparent text-forest border-2 border-forest font-bold px-8 py-5 w-full sm:w-auto text-sm uppercase tracking-widest flex justify-center items-center text-center leading-none hover:bg-forest hover:text-white transition-colors">
