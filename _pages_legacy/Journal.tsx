...
export default function Journal() {
  ...
  useEffect(() => {
    async function loadBlogs() {
      try {
        const data = await fetchCollection('blogs') as any[];
        const normalized = (Array.isArray(data) ? data : []).map((b: any) => ({
          id: b.id || b._id,
          slug: b.slug || String(b.title || '').toLowerCase().replace(/[^a-z0-9\s-]/g, '').trim().replace(/\s+/g, '-'),
          title: b.title || 'Untitled',
          excerpt: b.excerpt || b.summary || String(b.content || '').replace(/<[^>]+>/g, '').slice(0, 180),
          content: b.content || '',
          imageUrl: b.imageUrl || b.image_url || b.cover_image || '',
          status: String(b.status || '').toLowerCase(),
          createdAt: b.createdAt || b.created_at || b.published_at || new Date().toISOString(),
        }));

        const publishedBlogs = normalized
                ...
                <article key={blog.id} className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-slate-100 group flex flex-col h-full transform hover:-translate-y-1">
                  ...
                  <div className="p-6 flex flex-col flex-1">
                    ...
                    <div className="mt-auto flex items-center justify-between pt-4 border-t border-slate-100">
                      ...
                      <Link href={`/journal/${blog.slug || blog.id}`} className="flex items-center gap-1 text-sm font-bold text-forest group-hover:text-green-700 transition-colors">
         ...
         <div className="max-w-3xl mx-auto text-center relative z-10">
           ...
           <form className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto" onSubmit={(e) => { e.preventDefault(); alert('Subscribed successfully!'); }}>
             <input 
                ...
                required
               className="flex-1 px-4 py-3 rounded-lg bg-white text-slate-900 placeholder:text-slate-500 caret-slate-900 focus:outline-none focus:ring-2 focus:ring-saffron"
             />
